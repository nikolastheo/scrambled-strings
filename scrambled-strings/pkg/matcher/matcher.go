...
func CanonicalForm(word string) string {
	log.WithField("word", word).Info("Computing canonical form")
	
	// If the word has 2 or fewer characters, return it as-is
	if len(word) <= 2 {
		log.WithField("canonicalForm", word).Info("Word is 2 or fewer characters, returning as-is")
		return word
	}

	// Extract middle characters
	middle := []rune(word[1 : len(word)-1])
	log.WithField("middleCharacters", string(middle)).Debug("Extracted middle characters")

	// Sort the middle characters alphabetically
	sort.Slice(middle, func(i, j int) bool {
		return middle[i] < middle[j]
	})
	log.WithField("sortedMiddle", string(middle)).Debug("Sorted middle characters")
	...
	for _, word := range words {
		if _, exists := canonicalMap[word]; !exists {
			...
			log.WithFields(logrus.Fields{
				"word":          word,
				"canonicalForm": canonical,
			}).Debug("Added canonical form to dictionary map")
		} else {
			log.WithField("word", word).Debug("Skipped duplicate word in dictionary")
		}
	}
...
func CountMatches(dictionary map[string]string, input string) int {
	...
	for word, canonical := range dictionary {
		wordLen := len(word)
		log.WithField("word", word).Debug("Processing dictionary word")
		...
		for i := 0; i <= len(input)-wordLen; i++ {
			...
			}).Debug("Generated canonical form for substring")
